...
public class TaskSelector {
    ...
    private List<TaskTemplate> selectInterviewTasksByDaysRemaining(SelectionContext context, int maxDifficulty) {
        ...
        // 未过滤的全量池，用于难度过滤导致当天任务缺失时回退
        List<TaskTemplate> allUnfiltered = repository.findByCategory(TaskCategory.INTERVIEW_PREP);
        List<TaskTemplate> ordered = new ArrayList<>();
        int days = context.daysRemaining();

        if (days >= 6) {
            addIfPresent(ordered, all, allUnfiltered, "INTERVIEW_001");
            addIfPresent(ordered, all, allUnfiltered, "INTERVIEW_004");
        } else if (days >= 4) {
            addIfPresent(ordered, all, allUnfiltered, "INTERVIEW_002");
            addIfPresent(ordered, all, allUnfiltered, "INTERVIEW_003");
        } else if (days >= 2) {
            addIfPresent(ordered, all, allUnfiltered, "INTERVIEW_005");
            addIfPresent(ordered, all, allUnfiltered, "INTERVIEW_003");
        } else {
            addIfPresent(ordered, all, allUnfiltered, "INTERVIEW_006");
        }
    ...
    private List<TaskTemplate> selectExamTasksByDaysRemaining(SelectionContext context, int maxDifficulty) {
        ...
        List<TaskTemplate> allUnfiltered = repository.findByCategory(TaskCategory.EXAM_REVIEW);
        List<TaskTemplate> ordered = new ArrayList<>();
        int days = context.daysRemaining();

        if (days >= 6) {
            addIfPresent(ordered, all, allUnfiltered, "EXAM_001");
            addIfPresent(ordered, all, allUnfiltered, "EXAM_002");
        } else if (days >= 4) {
            addIfPresent(ordered, all, allUnfiltered, "EXAM_002");
            addIfPresent(ordered, all, allUnfiltered, "EXAM_003");
        } else if (days >= 2) {
            addIfPresent(ordered, all, allUnfiltered, "EXAM_003");
            addIfPresent(ordered, all, allUnfiltered, "EXAM_004");
        } else {
            addIfPresent(ordered, all, allUnfiltered, "EXAM_005");
        }
    ...
    private void addIfPresent(List<TaskTemplate> list, List<TaskTemplate> pool, List<TaskTemplate> fallbackPool, String templateId) {
        // 优先从难度过滤后的池中选取；若被过滤掉，则从全量池中回退选取（保证当天任务不缺失）
        TaskTemplate found = pool.stream()
                .filter(t -> t.templateId().equals(templateId))
                .findFirst()
                .orElse(null);
        if (found == null) {
            found = fallbackPool.stream()
                    .filter(t -> t.templateId().equals(templateId))
                    .findFirst()
                    .orElse(null);
        }
