...
import com.example.demo.m5.service.InMemoryTaskTemplateRepository;
...
class TaskTemplateTest {
    ...
    void taskGranularityIsDailyExecutable() {
        ...
        for (TaskTemplate t : all) {
            ...
            // 完成标准应该是具体可验收的，不是泛泛而谈
            String criteria = t.completionCriteria();
            assertFalse(criteria.contains("准备面试") && criteria.length() < 10,
                    "模板 " + t.templateId() + " 完成标准过于笼统");
        }
    }
    ...
    void bedtimeTemplatesAreLowArousal() {
        ...
        for (TaskTemplate t : bedtime) {
            assertEquals(com.example.demo.m5.model.TaskDifficulty.EASY, t.difficulty(),
                    "睡前任务 " + t.templateId() + " 应为EASY难度");
            assertEquals(com.example.demo.m5.model.TimeOfDay.BEDTIME, t.suggestedTime(),
